...
public final class ArrayQueue<T> implements QueueInterface<T> {
	...
	public void display() {
		// System.out.printf("Front Index: %s, [", frontIndex);
		...
		// System.out.printf("] Back Index: %s, %s", backIndex, Arrays.toString(queue));
		System.out.println();
	...
	public void splice(ArrayQueue<T> anotherQueue) {
		if (!anotherQueue.isEmpty()) {
			...
			while (anotherQueue.queue[index] != null) {
				enqueue(anotherQueue.queue[index]);
				index = (index + 1) % anotherQueue.queue.length;
			}
		}
	}
