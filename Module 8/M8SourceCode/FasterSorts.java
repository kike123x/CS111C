...
public class FasterSorts {
	...
	private static void mergeSortHelper(Comparable[] array, Comparable[] tempArray, int first, int last) {
		System.out.println("HELPER CALLED WITH First: " + first + " Last: " + last );
		if (first < last) {
			...
			if (array[mid].compareTo(array[mid+1]) > 0) { // efficiency improvement- when this fails, there is no need to merge
				...
				System.out.println("First: " + first + " Last: " + last + " " + Arrays.toString(array));
			}
		}
	}
	...
	private static void merge(Comparable[] array, Comparable[] tempArray, int first, int mid, int last) {
		...
		int endHalf2 = last;
		System.out.println("****** INSIDE MERGE -- FIRST: " + first + "-" + mid + ", SECOND: " + beginHalf2 + "-" + last);
	...
	private static int partition(Comparable[] array, int first, int last) {
		...
		swap(array, mid, last - 1); // Line A
		System.out.println(Arrays.toString(array));
		int pivotIndex = last - 1; // Line X
		...
		while (!done) {
			...
			if (indexFromLeft < indexFromRight) {
				swap(array, indexFromLeft, indexFromRight); // Line Y
				System.out.println("-----" + Arrays.toString(array));
				indexFromLeft++;
				indexFromRight--;
			} else
		...
		swap(array, pivotIndex, indexFromLeft); // Line Q
		System.out.println("******" + Arrays.toString(array));
